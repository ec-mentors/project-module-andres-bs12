...
export const FavoriteMealsMockupBar: React.FC<FavoriteMealsMockupBarProps> = ({
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-2 sm:px-4 mb-2 select-none animate-in fade-in duration-300">
      <div
        ref={barRef}
        className={`p-2 sm:p-2.5 rounded-2xl border flex flex-col gap-1.5 sm:gap-2 shadow-sm transition-all relative ${
          isLight
            ? 'bg-white/90 border-slate-200/90'
            : 'bg-[#121214]/90 border-white/[0.08]'
        }`}
      >
        {/* Section label — matches favoritesTypography.sectionLabel */}
        <div
          className={`flex items-center gap-1.5 px-1 ${
            isLight ? 'text-slate-600' : 'text-zinc-400'
          }`}
        >
          <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500/35 shrink-0" aria-hidden />
          <span className={`${favoritesTypography.sectionLabel} leading-none`}>
            My Favorites
          </span>
        </div>
      ...
      <ManageFavoritesModal
        ...
        onLogMeal={(payload) => {
          const match = favorites.find(
            (f) => f.mealName.trim().toLowerCase() === payload.mealName.trim().toLowerCase()
          );
          onSelectFavorite({
            id: match?.id ?? '',
            ...
            carbs: payload.carbs,
            fat: payload.fat,
            mealType: match?.mealType ?? getCurrentTimeMealType(),
